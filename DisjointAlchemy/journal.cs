...
public sealed class JournalLoader
{
	...
	public static List<PuzzleModelDisjoint> journal_puzzles = new();
	...
	public static void modifyJournal()
	{
		...
		{
			if (File.Exists(JModel.Path + "/Disjoint.journal.yaml"))
			{
		...
		{
			Logger.Log("[DisjointAlchemy] Could not find the 'Disjoint.journal.yaml' filepath amongst the loaded ModJournalModels:");
			foreach (var JModel in QuintessentialLoader.ModJournalModels)
			{
				Logger.Log("    " + JModel.Path);
			}
			throw new Exception("modifyJournalDisjoint: Content is missing.");
		}
		...
		Logger.Log("[DisjointAlchemy] Modifying journal items.");
		foreach (var volume in journal_self)
		{
			foreach (var puzzle in volume.field_2571)
			{
				...
				if (DisjointAlchemy.AdvancedContent.JournalRemappings.ContainsKey(puzzleID))
				{
					puzzleID = DisjointAlchemy.AdvancedContent.JournalRemappings[puzzleID];
				...
				foreach (var puzzleModel in DisjointAlchemy.AdvancedContent.Puzzles.Where(x => x.ID == puzzleID))
				{
	...
	{
		hook_JournalScreen_method_1040 = new Hook(DisjointAlchemy.PrivateMethod<JournalScreen>("method_1040"), OnJournalScreen_Method_1040);
	}
	...
	{
		var puzzleID = puzzle.field_2766;
		if (true) //(!DisjointAlchemy.AdvancedContent.Puzzles.Any(x => x.ID == puzzleID))
		{
			orig(screen_self, puzzle, basePosition, isLargePuzzle);
			return;
		}
		var puzzleModel = DisjointAlchemy.AdvancedContent.Puzzles.Where(x => x.ID == puzzleID).First();
		...
		{
			Song song = ModelWithResourcesDisjoint.fetchSong(puzzleModel.Music);
			Sound fanfare = ModelWithResourcesDisjoint.fetchSound(puzzleModel.Music);
